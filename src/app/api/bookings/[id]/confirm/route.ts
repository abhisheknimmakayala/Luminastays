...
export async function GET(
  ...
  if (booking.status === "confirmed") {
    // Count sibling rows so the page reflects the full group
    const totalRooms = await prisma.booking.count({
      where: {
    ...
    return html(successPage(booking, true, totalRooms));
  }

  // Confirm every pending booking in this group (same guest + same dates)
  const { count: roomsConfirmed } = await prisma.booking.updateMany({
    where: {
      guestPhone: booking.guestPhone,
      checkIn: booking.checkIn,
      checkOut: booking.checkOut,
  ...
  return html(successPage({ ...booking, status: "confirmed" }, false, roomsConfirmed));
}
...
function successPage(b: Booking, wasAlready: boolean, roomsConfirmed = 1) {
  ...
  const totalGuests = b.guests * roomsConfirmed;
  const roomsSummary = roomsConfirmed > 1 ? `${roomsConfirmed} Standard Rooms` : b.room.name;
...
<body>
  <div class="card">
    ...
    <div class="details">
      ...
      <div><strong>Rooms confirmed:</strong> ${roomsConfirmed}</div>
      <div><strong>Check-in:</strong> ${checkIn}</div>
      <div><strong>Check-out:</strong> ${checkOut}</div>
      <div><strong>Guests:</strong> ${b.guests * roomsConfirmed}</div>
    </div>
